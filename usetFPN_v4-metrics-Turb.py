...
# 1. Load data
path_output = 'resultados_marMenor-Turb_tabPFN'
# Ensure directory exists
os.makedirs(path_output, exist_ok=True)

df = pd.read_csv('dataset_examples/dataset_MarMenor-Tub.csv')
...
target_column = 'Mean_Turb_NTU'
if target_column not in df.columns:
    print(f"\nERROR: Target column '{target_column}' not found.")
    print("Available columns:", df.columns.tolist())
    # Search for possible target columns
    possible_targets = ['Mean_Turb_NTU', 'Turb', 'turbidity', 'Turbidity', 'Mean_Turb_NTU']
...
# ====================================================================
# ANALYSIS AND CORRECTION OF NEGATIVE TURBIDITY VALUES
# ====================================================================

print("\n" + "="*80)
print("ANALYSIS OF NEGATIVE TURBIDITY VALUES")
print("="*80)
...
print(f"Negative turbidity values: {num_negatives} ({percent_negatives:.1f}%)")
...
if num_negatives > 0:
    ...
    plt.title(f'Distribution of Negative Turbidity Values ({num_negatives} samples)', fontsize=14, fontweight='bold')
    plt.xlabel('Turbidity Value (NTU)', fontsize=14, fontweight='normal')
    ...
    plt.tight_layout()
    plt.savefig(f'{path_output}/distribution_negative_values_turb.pdf', dpi=150, bbox_inches='tight')
    plt.close()
#plt.show()
...
axes[0].set_title('Complete Turbidity Distribution', fontsize=14, fontweight='bold')
axes[0].set_xlabel('Turbidity (NTU)', fontsize=14, fontweight='normal')
...
axes[1].set_title(f'Zoom near zero [{zoom_range[0]}, {zoom_range[1]}] NTU', fontsize=14, fontweight='bold')
axes[1].set_xlabel('Turbidity (NTU)', fontsize=14, fontweight='normal')
...
plt.suptitle('ANALYSIS OF TURBIDITY VALUES - MAR MENOR', fontsize=16, fontweight='bold')
plt.tight_layout()
plt.savefig(f'{path_output}/turbidity_value_analysis.pdf', dpi=150, bbox_inches='tight')
plt.close()
#plt.show()
...
print("1. Replace with minimum positive value (0.001 NTU)")
...
if estrategia == 1:  # Replace with minimum positive value
    ...
    replacement_value = min(0.001, min_positive)  # Use 0.001 or the minimum positive, whichever is smaller
    print(f"Replacing negative values with {replacement_value:.4f} NTU")
    ...
    offset = abs(df[df[original_target_column] < 0][original_target_column].min()) + 0.001
    ...
    if original_target_column in numeric_cols:
        ...
        df_corrected.loc[df_corrected[original_target_column] < 0, original_target_column] = 0.001
...
if estrategia in [1, 3, 4]:  # Only for strategies that maintain original scale
    ...
    plt.tight_layout()
    plt.savefig(f'{path_output}/correction_comparison_turb_strategy_{estrategia}.pdf', dpi=150, bbox_inches='tight')
    plt.close()
#plt.show()
...
# ====================================================================
# ECOLOGICAL CATEGORIZATION BASED ON ACTUAL SCIENCE - MAR MENOR (TURBIDITY)
# ====================================================================
print("\n" + "="*80)
print("ECOLOGICAL CATEGORIZATION BASED ON SCIENCE - TURBIDITY")
print("="*80)
print("Based on: Pérez-Martín et al. (2023) - MMag Model of Mar Menor")
print("• Optimal/Robust State: < 1 NTU")
print("• High Risk of Mass Mortality: > 5 NTU")

# Ecological thresholds based on scientific literature of Mar Menor
umbrales_mar_menor = [1.0, 3.0, 5.0]  # Values in NTU according to Pérez-Martín et al. (2023)
etiquetas_ecologicas = ['Optimal', 'Surveillance', 'Precaution', 'High Mortality Risk']

# Create ecological categories
df['Turb_category_ecologica'] = pd.cut(df[target_column],
...
try:
    ...
    if df[target_column].nunique() > 10:
        df['Turb_category_estadistica'] = pd.qcut(df[target_column], q=4, labels=['Low', 'Medium-Low', 'Medium-High', 'High'])
    else:
        # If few unique values, use threshold-based categories
        thresholds = np.percentile(df[target_column], [25, 50, 75])
        df['Turb_category_estadistica'] = pd.cut(df[target_column], 
    ...
    dist_eco = df['Turb_category_ecologica'].value_counts().sort_index()
    for categoria, count in dist_eco.items():
        ...
        categoria_min = df[df['Turb_category_ecologica'] == categoria][target_column].min()
        categoria_max = df[df['Turb_category_ecologica'] == categoria][target_column].max()
        print(f"    Range: [{categoria_min:.2f} - {categoria_max:.2f}] NTU")

    print("\n2. DISTRIBUTION BY STATISTICAL CATEGORY (Quartiles):")
    if 'Turb_category_estadistica' in df.columns:
        dist_est = df['Turb_category_estadistica'].value_counts().sort_index()
    ...
    print("\n" + "="*80)
    print("VERIFICATION OF CRITICAL STATES - MAR MENOR (TURBIDITY)")
    print("="*80)

    # Count samples in high risk state (> 5 NTU)
    alto_riesgo_count = (df[target_column] > 5.0).sum()
    alto_riesgo_porcentaje = (alto_riesgo_count / len(df)) * 100
    print(f"Samples in HIGH MORTALITY RISK (> 5 NTU): {alto_riesgo_count} ({alto_riesgo_porcentaje:.1f}%)")

    # Count samples in optimal state (< 1 NTU)
    optimo_count = (df[target_column] < 1.0).sum()
    optimo_porcentaje = (optimo_count / len(df)) * 100
    print(f"Samples in OPTIMAL/ROBUST state (< 1 NTU): {optimo_count} ({optimo_porcentaje:.1f}%)")
    ...
    if 'Turb_category_ecologica' in df.columns:
        ...
        #axes[0].set_title('ECOLOGICAL Classification', fontsize=14, fontweight='bold')
    ...
    if 'Turb_category_estadistica' in df.columns:
        ...
        #axes[1].set_title('STATISTICAL Categorization (Quartiles)', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(f'{path_output}/categorization_comparison_mar_menor_turb.pdf', dpi=150, bbox_inches='tight')
    plt.close()
#plt.show()
    ...
    print("="*80)
    df['Turb_category'] = df['Turb_category_ecologica']  # Use ecological for modeling

    print("\nTurbidity category distribution:")
    print(df['Turb_category'].value_counts())

    # Check balance
    category_counts = df['Turb_category'].value_counts()
    ...
    df['Turb_category'] = np.where(df[target_column] > median_val, 'High', 'Low')
    print("Binary category distribution:")
    print(df['Turb_category'].value_counts())

# ====================================================================
# NEW FUNCTIONS FOR FIGURES 3b, 4c, AND 5 (ADAPTED FROM CHL SCRIPT)
# ====================================================================

def crear_learning_curves_individual(modelo_nombre, modelo, X_train, y_train, X_val, y_val, path_output, variable_name="Turbidity"):
    """
...
def crear_learning_curves_all_models(modelos_dict, X_train, y_train, X_val, y_val, path_output, variable_name="Turbidity"):
    """
...
def crear_feature_importance(modelo, X_train, y_train, feature_names, path_output, modelo_nombre, variable_name="Turbidity"):
    """
...
def crear_feature_importance_tabpfn(modelo, X_train, y_train, feature_names, path_output, modelo_nombre, variable_name="Turbidity", n_repeats=5, n_samples=1000):
    """
...
def crear_temporal_decomposition(df, target_column, path_output, variable_name="Turbidity"):
    """
    Creates temporal decomposition plot showing seasonal and annual patterns.
    Figure 4c: Temporal decomposition of turbidity time series.
    """
    ...
    ax1.set_ylabel('Turbidity (NTU)', fontsize=14, fontweight='normal')
    ...
    ax2.set_ylabel('Average Turbidity (NTU)', fontsize=14, fontweight='normal')
    ...
    if has_yearly and ax3 is not None:
        ...
        if len(years) > 0 and np.sum(yearly_values) > 0:
            ...
            ax3.set_ylabel('Annual Average Turbidity (NTU)', fontsize=14, fontweight='normal')
            ...
            if len(yearly_values) >= 2:
                try:
                    ...
                    if np.sum(valid_idx) >= 2:
                        ...
                        trend_text = f'Annual Trend: {z_yearly[0]:.3f} NTU/year'
...
def obtener_metricas_computacionales(modelo, X_train, y_train, X_test, y_test, modelo_nombre, tipo_modelo='auto'):
    ...
    if tipo_modelo == 'classification':
        ...
        metricas['mse'] = None
        metricas['mae'] = None
        metricas['r2_score'] = None #modificado
        ...
        metricas['r2_score'] = r2_score(y_test, y_pred) #modificado
...
def crear_scatter_regresion(y_true, y_pred, modelo_nombre, target_name):
    ...
    #ax1.set_title(f'{modelo_nombre} - Predicted vs Actual', fontsize=14, fontweight='bold')
    ...
    #plt.suptitle(f'REGRESSION DIAGNOSTICS - {target_name}', fontsize=16, fontweight='bold', y=0.98)
    plt.tight_layout()
...
def crear_visualizaciones_eda(df, target_column='Mean_Turb_NTU'):
    """Creates exploratory visualizations of Mar Menor dataset in R style"""
    print("\nGenerating EDA visualizations for Mar Menor (Turbidity)...")
    
    # Create figure with subplots
    fig = plt.figure(figsize=(20, 15))
    
    # 1. Temporal distribution of turbidity
    ...
    if 'TIMESTAMP' in df.columns:
        ...
        ax1.set_title('Temporal Evolution of Turbidity', fontsize=14, fontweight='bold')
        ax1.set_xlabel('Date', fontsize=14, fontweight='normal')
        ax1.set_ylabel('Turbidity (NTU)', fontsize=14, fontweight='normal')
        ax1.grid(True, axis='y', color='black', linestyle=':', linewidth=0.3, alpha=1.0)
        for spine in ['top', 'bottom', 'left', 'right']:
    ...
    # 2. Turbidity distribution
    ax2 = plt.subplot(3, 3, 2)
    ax2.set_facecolor('white')
    ax2.hist(df[target_column], bins=30, alpha=0.7, color='sandybrown', edgecolor='black', linewidth=0.8)
    ax2.set_title('Turbidity Distribution', fontsize=14, fontweight='bold')
    ax2.set_xlabel('Turbidity (NTU)', fontsize=14, fontweight='normal')
    ...
    # 4. Relationship between water temperature and turbidity
    ...
    if 'SDI_Temp_3m' in df.columns:
        ...
        ax4.set_title('Water Temperature vs Turbidity', fontsize=14, fontweight='normal')
        ax4.set_xlabel('Water Temperature (°C)', fontsize=11)
        ax4.set_ylabel('Turbidity (NTU)', fontsize=11)
    ...
    # 5. Relationship between oxygen and turbidity
    ...
    if 'O2_sat2_Avg' in df.columns:
        ...
        ax5.set_title('Dissolved Oxygen vs Turbidity', fontsize=14, fontweight='normal')
        ax5.set_xlabel('Oxygen Saturation (%)', fontsize=11)
        ax5.set_ylabel('Turbidity (NTU)', fontsize=11)
    ...
    # 6. Relationship between conductivity and turbidity
    ...
    if 'SDI_TempCorrCond_3m' in df.columns:
        ...
        ax6.set_title('Conductivity vs Turbidity', fontsize=14, fontweight='normal')
        ax6.set_xlabel('Corrected Conductivity', fontsize=11)
        ax6.set_ylabel('Turbidity (NTU)', fontsize=11)
    ...
    # 7. Relationship between wind speed and turbidity
    ...
    if 'WS_ms_Avg' in df.columns:
        ...
        ax7.set_title('Wind Speed vs Turbidity', fontsize=14, fontweight='normal')
        ax7.set_xlabel('Wind Speed (m/s)', fontsize=11)
        ax7.set_ylabel('Turbidity (NTU)', fontsize=11)
    ...
    # 8. Daily variation of turbidity
    ax8 = plt.subplot(3, 3, 8)
    if 'hour' in df.columns:
        ax8.set_facecolor('white')
        hourly_avg = df.groupby('hour')[target_column].mean()
        ax8.plot(hourly_avg.index, hourly_avg.values, 
                color='saddlebrown', linewidth=2, marker='o', markersize=6, markerfacecolor='white', markeredgecolor='black', markeredgewidth=0.8)
        ax8.fill_between(hourly_avg.index, hourly_avg.values, 
                        alpha=0.3, color='sandybrown')
        ax8.set_title('Hourly Turbidity Variation', fontsize=14, fontweight='normal')
        ax8.set_xlabel('Hour of Day', fontsize=11)
        ax8.set_ylabel('Average Turbidity (NTU)', fontsize=11)
    ...
    # 9. Turbidity categories
    ax9 = plt.subplot(3, 3, 9)
    if 'Turb_category' in df.columns:
        ax9.set_facecolor('white')
        counts = df['Turb_category'].value_counts()
        ...
        ax9.set_title('Turbidity Categories', fontsize=14, fontweight='normal')
    ...
    plt.suptitle('EXPLORATORY DATA ANALYSIS - MAR MENOR (Turbidity)', 
                fontsize=18, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(f'{path_output}/eda_mar_menor_turb.pdf', dpi=150, bbox_inches='tight')
    plt.close()
#plt.show()
...
def visualizar_metricas_computacionales(metricas_modelos):
    ...
    plt.suptitle('COMPUTATIONAL METRICS - MAR MENOR (TURBIDITY)', 
                fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(f'{path_output}/computational_metrics_mar_menor_turb.pdf', dpi=150, bbox_inches='tight')
    plt.close()
#plt.show()
...
def crear_tabla_resumen_metricas(metricas_modelos):
    ...
    """
    print("\n" + "="*80)
    print("COMPUTATIONAL METRICS SUMMARY - MAR MENOR (TURBIDITY)")
    print("="*80)
    ...
    # Save to CSV
    df_resumen.to_csv(f'{path_output}/metrics_summary_mar_menor_turb.csv', index=False)
    print(f"\n✓ Summary saved to 'metrics_summary_mar_menor_turb.csv'")
...
def crear_grafico_comparativo_modelos(resultados_modelos):
    ...
    plt.suptitle('COMPARATIVE ANALYSIS OF MODELS - MAR MENOR (TURBIDITY)', fontsize=16, fontweight='bold', y=1.05)
    plt.tight_layout()
    plt.savefig(f'{path_output}/model_comparison_mar_menor_turb.pdf', dpi=150, bbox_inches='tight')
    plt.close()
#plt.show()
...
def crear_grafico_comparativo_modelos_clasificacion(resultados_modelos):
    ...
    plt.tight_layout()
    plt.savefig(f'{path_output}/model_comparison_classification_mar_menor_turb.pdf', dpi=150, bbox_inches='tight')
    plt.close()
#plt.show()
...
def crear_grafico_comparativo_modelos_regresion(resultados_modelos):
    ...
    plt.tight_layout()
    plt.savefig(f'{path_output}/model_comparison_regression_mar_menor_turb.pdf', dpi=150, bbox_inches='tight')
    plt.close()
#plt.show()
...
def mostrar_matriz_confusion(y_real, y_pred, titulo="Confusion Matrix", modelo_nombre=""):
    ...
    #ax1.set_title(f'Confusion Matrix - {modelo_nombre}', fontsize=14, fontweight='bold', pad=20)
    ...
    filename_cm = f'confusion_matrix_{modelo_nombre.replace(" ", "_").lower()}_mar_menor_turb.pdf'
    ...
    filename_metrics = f'perclass_metrics_{modelo_nombre.replace(" ", "_").lower()}_mar_menor_turb.pdf'
...
def crear_curvas_calibracion(modelos_clasificacion, X_test, y_test, y_classes, path_output, variable_name="Turbidity"):
    """
    ...
    for modelo_nombre, modelo_info in modelos_clasificacion.items():
        print(f'modelonombres====>{modelo_nombre}')
    ...
    #ax.set_title(f'Calibration Curves - {variable_name}', fontsize=14, fontweight='bold')
...
def crear_grafico_time_accuracy_tradeoff(metricas_todos_modelos, resultados_globales, path_output, variable_name="Turbidity"):
    """
    ...
    #ax.set_title(f'Time-Accuracy Tradeoff - {variable_name}', fontsize=16, fontweight='bold')
    ax.set_xscale('log')
...
def crear_radar_chart_top5(resultados_globales, path_output, variable_name="Turbidity"):
    """
...
# ====================================================================
# MAIN CODE MODIFIED FOR MAR MENOR - TURBIDITY
# ====================================================================

# Additional exploratory analysis
print("\n" + "="*50)
print("EXPLORATORY DATA ANALYSIS - MAR MENOR (TURBIDITY)")
print("="*50)
print("\nDescriptive statistics of key variables:")
variables_clave = ['Air_Temp_HS_Avg', 'WS_ms_Avg', 'O2_sat2_Avg', 
                   'SDI_Temp_3m', 'SDI_TempCorrCond_3m', 
                   target_column]
...
# Option 1: Classification (turbidity categories)
if 'Turb_category' in df.columns:
    y_class = df['Turb_category']
    ...
    y_class_encoded, y_classes = pd.factorize(y_class)
    df['Turb_category'] = y_class

# Option 2: Regression (continuous turbidity values)
y_reg = df[target_column]

# Select features (exclude non-numeric or temporal columns)
exclude_cols = ['TIMESTAMP', target_column, 'Turb_category', 'hour', 'day', 'month', 'year', 'dayofyear', 'censored'] if 'censored' in df.columns else ['TIMESTAMP', target_column, 'Turb_category', 'hour', 'day', 'month', 'year', 'dayofyear']
...
def guardar_modelo(modelo, scaler, nombre_archivo='modelo_mar_menor_turb.pkl'):
...
def cargar_modelo(nombre_archivo='modelo_mar_menor_turb.pkl'):
...
def entrenar_modelo_regresion(X_train, y_train, X_val, y_val, X_test, y_test, metricas_globales, resultados_globales):
    ...
    print("\n" + "="*50)
    print("REGRESSION MODELS - TURBIDITY (Continuous Values)")
    print("="*50)
...
def entrenar_modelo_clasificacion(X_train, y_train, X_val, y_val, X_test, y_test, metricas_globales, resultados_globales):
    ...
    print("\n" + "="*50)
    print("CLASSIFICATION MODELS - TURBIDITY CATEGORIES")
    print("="*50)
    ...
    try:
        ...
        if tipo == 'classification':
            ...
            try:
                ...
                plt.tight_layout()
                plt.savefig(f'{path_output}/smote_balancing_mar_menor_turb.pdf', dpi=150, bbox_inches='tight')
                plt.close()
#plt.show()
        ...
        plt.tight_layout()
        plt.savefig(f'{path_output}/improvement_comparison_{tipo}_turb.pdf', dpi=150, bbox_inches='tight')
        plt.close()
#plt.show()
...
try:
    ...
    # Save TabPFN model
    guardar_modelo(model_tabpfn, scaler_class, 'tabpfn_model_mar_menor_turb.pkl')
...
try:
    ...
    guardar_modelo(model_tabpfn_reg, scaler_reg, 'tabpfn_reg_model_mar_menor_turb.pkl')
...
fig_td, daily_avg, monthly_avg, yearly_avg = crear_temporal_decomposition(
    df, target_column, path_output, variable_name="Turbidity"
)
...
colors = ['#377EB8' if t == 'classification' else '#E41A1C' for t in tipos]  # R ColorBrewer colors
...
ax.set_ylabel('Accuracy', fontsize=14, fontweight='normal')
ax.set_title('Time-Accuracy Tradeoff by Model - Mar Menor (Turbidity)', fontsize=16, fontweight='bold')
ax.grid(True, axis='both', color='black', linestyle=':', linewidth=0.3, alpha=1.0)
ax.set_axisbelow(True)
for spine in ['top', 'bottom', 'left', 'right']:
    ax.spines[spine].set_color('black')
...
plt.tight_layout()
plt.savefig(f'{path_output}/time_accuracy_tradeoff_mar_menor_turb.pdf', dpi=150, bbox_inches='tight')
plt.close()
#plt.show()

print("\n✓ Tradeoff analysis saved to 'time_accuracy_tradeoff_mar_menor_turb.pdf'")
...
print("\n" + "="*80)
print("FINAL EXECUTIVE SUMMARY - MAR MENOR (TURBIDITY)")
print("="*80)
...
with open(f'{path_output}/complete_results_mar_menor_turb.pkl', 'wb') as f:
...
print("="*80)
print("\n📊 FILES GENERATED:")
print("  1. eda_mar_menor_turb.pdf                      - Exploratory data analysis")
print("  2. computational_metrics_mar_menor_turb.pdf    - Metrics comparison (includes NLL and ECE)")
print("  3. model_comparison_mar_menor_turb.pdf         - Model comparison")
print("  4. time_accuracy_tradeoff_mar_menor_turb.pdf   - Time-accuracy analysis")
print("  5. metrics_summary_mar_menor_turb.csv          - Metrics table (includes NLL and ECE)")
print("  6. confusion_matrix_*_mar_menor_turb.pdf       - Confusion matrices")
print("  7. smote_balancing_mar_menor_turb.pdf          - SMOTE balancing")
print("  8. improvement_comparison_*_turb.pdf           - Improved models")
print("  9. complete_results_mar_menor_turb.pkl         - Serialized data")
print(" 10. tabpfn_model_mar_menor_turb.pkl             - TabPFN model saved")
print(" 11. tabpfn_reg_model_mar_menor_turb.pkl         - TabPFN Regressor model saved")
print(" 12. regression_scatter_*.pdf                    - Predicted vs Actual scatter plots")
print("\n📈 SCIENTIFIC FIGURES FOR PUBLICATION:")
print(" 13. learning_curve_*_turb.pdf                   - Learning curves (Figure 3b) [NEW]")
print(" 14. figure4c_temporal_decomposition_turb.pdf    - Temporal decomposition (Figure 4c) [NEW]")
print(" 15. figure5_feature_importance_turb.pdf         - Feature importance (Figure 5) [NEW]")
print(" 16. figure4_calibration_curves_turb.pdf         - Calibration curves (Figure 4)")
print(" 17. figure5_time_accuracy_tradeoff_turb.pdf     - Time-accuracy tradeoff (Figure 5)")
print(" 18. figure6_radar_chart_turb.pdf                - Radar chart top 5 models (Figure 6)")
...
print("  • Feature importance (top features affecting turbidity)")
...
print("="*80)
print(f"• Dataset: Mar Menor - Turbidity")
print(f"• Total samples: {len(df)}")
print(f"• Features: {len(feature_cols)}")
print(f"• Target variable: {target_column}")
print(f"• Turbidity range: {df[target_column].min():.2f} - {df[target_column].max():.2f} NTU")
